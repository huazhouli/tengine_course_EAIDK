...
int PrintTopLabels_cv(const char* label_file, float* data, int data_size,const char* image_file)
{
    ...
    //sprintf(result_buffer,"%s %f%",labels[top_N[0]],result[top_N[0]]);
    cv::putText(img, labels[top_N[0]], cv::Point(100, 300),cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 0)); //图片上写字
    imshow("image", img);
    waitKey();
}
...
int main(int argc, char* argv[])
{
    ...
    /* after process */
    PrintTopLabels_cv(_label_file, (float*)output_tensor.data, 1000,_image_file);
    std::cout << "--------------------------------------\n";
