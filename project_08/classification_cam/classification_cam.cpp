...
std::string str_N[3];       //识别结果
int labe_num = 0;           //标签个数

const string wintitle = "classification_cam";
int PrintTopLabels_cv(const char* label_file, float* data, int data_size)
{
    ...
    std::vector<int> top_N = Argmax(result, 1);

    for(unsigned int i = 0; i < top_N.size(); i++)
    {
        int idx = top_N[i];
		sprintf(result_buf,"%.3f-", result[idx]);
		str_N[i] = result_buf + labels[idx].substr(10, labels[idx].length());
...
int main(int argc, char* argv[])
{
    ...
    /* after process */
    PrintTopLabels_cv(_label_file, (float*)output_tensor.data, 1000);
	if (labe_num)
	{	
	for (int i = 0; i < labe_num; i++)
	{	fcv::putText(image, str_N[i], fcv::Point(100, 300),cv::FONT_HERSHEY_SIMPLEX, 1, fcv::Scalar(0, 255, 0)); //图片上写字
